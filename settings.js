...
const{settingsMyteamUrl,staffFirstName,staffLastName,staffName_NotIntheList,staffEmailID,staffFirstNameInAltCaps,staffFirstNameFirstLetter,staffLastNameFirstLetter,
   staffPhonenumber,staffRole,staffPermissionLevel}=require("./myTeamData")
...
async checkStaffAddedInMyTeam() {
   ...
   const locator =  this.page.locator('xpath=.//div[@class="peopleList awd-custom-scrollbar"]');
   await expect(locator).toContainText(new RegExp(this.staffName, 'i'));
}
...
async deleteStaff() {
   await this.page.locator('xpath=(.//div[@class="fx-v people-listitem "])[1]').click();
