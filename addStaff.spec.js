...
const{setmoreSettingsPage}=require("../settings")
const{setmoreLoginPage}=require('../login')
...
test('add staff with Enchanced permission', async({page})=>{
    const settingsPage = new setmoreSettingsPage(page,"john","hapi001@mailinator.com")
...
test('add staff with No Access permission', async({page})=>{
    const settingsPage = new setmoreSettingsPage(page,"kevin","hapi002@mailinator.com")
    await settingsPage.gotoSettingsMyteam();
    await settingsPage.clickAddStaffButton();
    await settingsPage.enterFullNameInAddStaffPopup();
    await settingsPage.enterEmailIdInAddStaffPopup();
...
test('add staff with Standard permission', async({page})=>{
    const settingsPage = new setmoreSettingsPage(page,"roger","hapi777@mailinator.com")
    await settingsPage.gotoSettingsMyteam();
    await settingsPage.clickAddStaffButton();
    await settingsPage.enterFullNameInAddStaffPopup();
    await settingsPage.enterEmailIdInAddStaffPopup();
...
test('add staff when staff name starts with empty spaces', async({page})=>{
    const settingsPage = new setmoreSettingsPage(page,"   spaceX","hapi015@mailinator.com")
    await settingsPage.gotoSettingsMyteam();
    await settingsPage.clickAddStaffButton();
    await settingsPage.enterFullNameInAddStaffPopup();
    await settingsPage.enterEmailIdInAddStaffPopup();
...
test.only('delete the existing staff and add a new staff', async({page})=>{
    const settingsPage = new setmoreSettingsPage(page,"kevin","ape199900@mailinator.com")
